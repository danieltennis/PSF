...
void MeetingPlanner::addMeeting(const std::string &label, const std::string &id, const Room &room,
                                const std::string &dateStr, bool catering, bool online) {
    int y, m, d;
    char dash;
...
int MeetingPlanner::getMeetingCO2(const Meeting &m) {
    if (m.isOnline()) {
        return 5; // online meeting
    }
    return 10; // fysieke meeting
}

int MeetingPlanner::getParticipantCount(const std::string &meetingId) const {
    int count = 0;
    for (const auto &p: participations) {
        if (p.getMeetingIdentifier() == meetingId)
            count++;
    }
    return count;
}
...
void MeetingPlanner::writeAll(std::ostream &out) const {
    ...
    if (meetings.empty()) out << "Geen meetings gevonden." << endl;
        ...
        for (const auto &m: meetings) {
            ...
            int meetingCO2 = getMeetingCO2(m);
            int cateringCO2 = 0;
            if (m.hasCatering() && !m.isOnline()) {
                int perPerson = getCO2ForCampus(m.getRoom().getCampus());
                int participants = getParticipantCount(m.getIdentifier());
                cateringCO2 = perPerson * participants;
            }

            out << " CO2 meeting: " << meetingCO2 << " kg" << endl;
            out << " CO2 catering: " << cateringCO2 << " kg" << endl;
    ...
    for (const auto &m: meetings) {
        ...
        totalCO2 += meetingCO2 + cateringCO2;
    }
